      ...
      <div style="max-width: 600px; margin: 0 auto;">
        ...
        <div style="background: linear-gradient(135deg, #1e3a5f 0%, #0f172a 100%); border-radius: 16px; padding: 40px; box-shadow: 0 20px 40px rgba(0,0,0,0.3); border: 1px solid rgba(245, 158, 11, 0.2);">
          ...
          <div style="background: rgba(255,255,255,0.05); border-radius: 12px; padding: 24px; margin-bottom: 24px;">
            ...
            <p style="color: #cbd5e1; font-size: 15px; line-height: 24px; margin: 0;">
              This is a friendly reminder that you registered for the following event happening tomorrow:
            </p>
          </div>
        ...
        <div style="text-align: center; margin-top: 32px; padding-top: 24px;">
          <p style="color: #64748b; font-size: 12px; margin: 0 0 8px 0;">
            You're receiving this reminder because you registered for this event.
          </p>
  ...
  try {
    ...
    for (const event of upcomingEvents) {
      ...
      if (rsvpError) {
        ...
        console.log(`Found ${rsvps.length} member RSVP(s) for event: ${event.title}`);
        ...
        if (usersToRemind.length > 0) {
          console.log(`Need to remind ${usersToRemind.length} member(s) for event: ${event.title}`);
          ...
          for (const profile of profiles || []) {
            ...
            try {
              ...
              totalRemindersSent++;
              console.log(`Member reminder sent to ${email} for event: ${event.title}`);
            } catch (emailError) {
              console.error(`Failed to send reminder to ${email}:`, emailError);
            }
          }
        } else {
          console.log(`All members already reminded for event: ${event.title}`);
        }
      }

      // ========== GUEST REMINDERS ==========
      // Get guest RSVPs for this event
      const { data: guestRsvps, error: guestError } = await supabaseAdmin
        .from('guest_event_rsvps')
        .select('full_name, email')
        .eq('event_id', event.id);

      if (guestError) {
        console.error(`Error fetching guest RSVPs for event ${event.id}:`, guestError);
        continue;
      }

      if (guestRsvps && guestRsvps.length > 0) {
        console.log(`Found ${guestRsvps.length} guest RSVP(s) for event: ${event.title}`);

        // Check which guests have already received reminders
        const { data: sentGuestReminders } = await supabaseAdmin
          .from('guest_event_reminders_sent')
          .select('guest_email')
          .eq('event_id', event.id);

        const alreadySentGuestEmails = new Set(sentGuestReminders?.map(r => r.guest_email) || []);

        // Filter guests who haven't received reminders yet
        const guestsToRemind = guestRsvps.filter(g => !alreadySentGuestEmails.has(g.email));

        if (guestsToRemind.length > 0) {
          console.log(`Need to remind ${guestsToRemind.length} guest(s) for event: ${event.title}`);
          ...
          for (const guest of guestsToRemind) {
            ...
            try {
              ...
              totalRemindersSent++;
              console.log(`Guest reminder sent to ${guest.email} for event: ${event.title}`);
            } catch (emailError) {
              console.error(`Failed to send guest reminder to ${guest.email}:`, emailError);
            }
          }
        } else {
          console.log(`All guests already reminded for event: ${event.title}`);
        }
      }
    }
